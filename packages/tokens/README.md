...
|---|---|---|
| （`:root` 預設） | — | 中性深色（藍黑） |
| `.al-theme-blueprint` | **系列預設**（原 ArchLens Diff 招牌） | 藍圖 |
| `.al-theme-hacker` | Web Hacker 模式 / SuiteNav 品牌 | 駭客綠 |
| `.al-theme-light` | （已自系列移除，CSS 定義保留供未來復用） | 淺色 |

新增主題包：複製一個 class、覆寫顏色角色即可（結構 token 不要動）。

### 系列慣例（重要）

- **預設主題＝`.al-theme-blueprint`**，四個產品 + hub（含未來新產品）一律以 Blueprint 為預設。
  原本的 Light 預設已移除——其配色難收斂到乾淨舒適的狀態。`.al-theme-light` 的 CSS 定義
  仍保留於 `tokens.css`（未來若要復用不必重寫），但已從各產品的切換器拿掉、不再是預設。
- **主題選擇不持久化**：切換器只在當次 session 生效，**不寫 localStorage**；每次重新載入都重置回 Blueprint。這是刻意的隱私選擇（不在使用者裝置留存任何偏好），與系列「隱私優先、無後端、不留存」一致。
- 因此各產品 `index.html` 直接在 `<html>` 掛 `class="al-theme-blueprint"`（靜態、無腳本、無讀取），避免閃爍也不需儲存。
- **ArchLens Web 例外**：已收斂為單一 Blueprint 介面、移除主題切換（舊的 Light/Hacker 兩套元件樹移至該 repo 的 `_archive/`）。
